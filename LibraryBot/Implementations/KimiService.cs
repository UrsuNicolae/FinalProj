...
namespace LibraryBot.Implementations
{
    public class KimiService : IKimiService
    {
        ...
        public KimiService(HttpClient httpClient, IConfiguration configuration)
        {
        ...
        public async Task<string> GetBookRecommandations(BookDto book, AuthorDto authorDto, CategoryDto categoryDto, int count, CancellationToken ct)
        {
            ...
            return await SendKimiRequest(prompt, ct);
        }

        private async Task<string> SendKimiRequest(string prompt, CancellationToken ct)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new Exception($"Kimi returned status code: {response.StatusCode}. Body: {error}");
            }
            var responseJson = await response.Content.ReadAsStringAsync(ct);
            var kimiResponse = JsonSerializer.Deserialize<KimiResponse>(responseJson, options);
            return kimiResponse?.Choices.FirstOrDefault()?.Message.Content;
        }
